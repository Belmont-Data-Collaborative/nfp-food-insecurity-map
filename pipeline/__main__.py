...
Usage:
    ...
    python -m pipeline --step food_insecurity_index # Pull the published Food Insecurity Index from S3
...
def run_data_step(source_key: str) -> None:
    ...
    for granularity in ["tract", "zip"]:
        if granularity == "zip" and source_config.get("tract_only"):
            logger.debug("%s is tract_only — skipping zip", source_key)
            continue
